"""
CSA51 - Program 26: DES Implementation
Full DES encryption/decryption implementation from scratch.
"""
...
def s_box(bits):
    ...
    for i in range(8):
        ...
        result += [(val >> j) & 1 for j in range(3, -1, -1)]
    return result
...
def main():
    ...
    print("=" * 60)
    print("Note: Uses 8-char plaintext/key (64-bit each)")
    print()
    print("1. Encrypt")
    print("2. Decrypt")
    ...
    if choice == '1':
        pt = input("Enter 8-char plaintext: ")
        key = input("Enter 8-char key     : ")
        if len(pt) < 8: pt = pt.ljust(8)[:8]
        if len(key) < 8: key = key.ljust(8)[:8]

        pt_bits  = str_to_bits(pt)
        key_bits = str_to_bits(key)
        subkeys  = generate_subkeys(key_bits)
        ct_bits  = des_encrypt_block(pt_bits, subkeys)
        ...
        ct_hex = input("Enter ciphertext (16 hex chars): ")
        key    = input("Enter 8-char key               : ")
        if len(key) < 8: key = key.ljust(8)[:8]

        ct_bits  = hex_to_bits(ct_hex, 64)
        key_bits = str_to_bits(key)
        subkeys  = generate_subkeys(key_bits)
        pt_bits  = des_decrypt_block(ct_bits, subkeys)
        pt       = bits_to_str(pt_bits)
